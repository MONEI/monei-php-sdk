    ...
    public static function deserialize($data, $class, $httpHeaders = null)
    {
        ...
        if ($class === '\SplFileObject') {
            ...
            if (is_array($httpHeaders) && array_key_exists('Content-Disposition', $httpHeaders) && preg_match('/inline; filename=[\'"]?([^\'"\s]+)[\'"]?$/i', $httpHeaders['Content-Disposition'], $match)) {
                $filename = \Monei\Configuration::getDefaultConfiguration()->getTempFolderPath() . \DIRECTORY_SEPARATOR . self::sanitizeFilename($match[1]);
            } else {
                $filename = tempnam(\Monei\Configuration::getDefaultConfiguration()->getTempFolderPath(), '');
            }
        ...
        if (method_exists($class, 'getAllowableEnumValues')) {
            ...
            if (!empty($discriminator) && isset($data->{$discriminator}) && is_string($data->{$discriminator})) {
                $subclass = '\Monei\Model\\' . $data->{$discriminator};
